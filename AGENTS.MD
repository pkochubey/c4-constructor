# C4 Visual Constructor - AI Agent Guide

This guide helps AI agents understand the project architecture, patterns, and conventions for consistent development.

## Project Overview

**C4 Visual Constructor** is a visual diagram editor for C4 architecture models with Structurizr DSL import/export. Built with React, TypeScript, ReactFlow, and Zustand.
...
interface WorkspaceState {
  ...
  // CRUD operations
  addElement, updateElement, deleteElement, renameElement
  addRelationship, updateRelationship, deleteRelationship
  addView, updateView, deleteView, setCurrentView
...
- The store auto-creates views when adding Software Systems or Deployment Nodes
...
type C4ElementType =
  ...
  | 'infrastructureNode' // Infrastructure (cloud, server, etc.)
...
| `deployment` | Deployment structure | `deploymentNodeId` |
...
- Container → Component
- Deployment Node → Deployment Node, Infrastructure Node, Container
- Group → Any element (visual grouping only)

**Position handling:**
- Child positions are **relative to parent** in store
- Canvas converts to absolute for display (unless "sticky" in landscape)
- "Sticky" nodes use ReactFlow's `parentNode` prop
...
```typescript
const allowedTypes = {
  systemLandscape: ['person', 'softwareSystem', 'deploymentNode', 'group'],
  systemContext: ['person', 'softwareSystem', 'group'],
  container: ['container', 'person', 'group'],
  component: ['component', 'person', 'group'],
  deployment: ['deploymentNode', 'infrastructureNode', 'container', 'group']
}[currentView.type];
```
...
{selectedElement && (
  <div key={selectedElement.id} className="flex flex-col gap-5">
    <Input value={name} onChange={(e) => {
      ...
      if (currentElement) updateElement(currentElement.id, { name: e.target.value });
    }} />
  </div>
)}
```
...
- Generates all views (landscape, context, container, component, deployment)
...
const C4_COLORS = {
  ...
  deploymentNode: { background: '#cccccc', border: '#999999' },
  // ...
};
```
